...
const MigracionButtonSegEjecutoresA = () => {
  const ejecutarMigracion = async () => {
    try {
      const token = localStorage.getItem(ACCESS_TOKEN); // Obtén el token de autorización almacenado en localStorage
      const response = await axios.post('http://localhost:8000/api/migrar-datos-seg-ejecutores-a/', {}, {
        headers: {
...
export default MigracionButtonSegEjecutoresA;