...
csv_folder_path = os.path.join(project_root, 'files', 'seguimiento_ejecutor_b_csv')
...
c.execute("DELETE FROM api_seguimientoejecutoresb")
    ...
    if filename.endswith('.csv'):
        ...
        with open(csv_file_path, 'r', encoding='latin-1') as csv_file:
            ...
            for row in csv_reader:
                ...
                c.execute("INSERT OR IGNORE INTO api_seguimientoejecutoresb VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
